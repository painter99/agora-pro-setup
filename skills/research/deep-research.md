...
Multi-source web research with ReAct loop, source scoring, quality gates, and iteration bounds. Load when a task needs 2+ sources, fact-checking, comparison, or consequential verification.

> Install into Agora **Saved Skills** as `deep-research` (flat name).
> Add the catalog description above — that is what `{skill_catalog}` shows.
> Do **not** also store this file as a Saved Memory.

### Agora 2.1 notes

- Discovery is `{skill_catalog}`, not an Active Memory Module Registry.
- Read this Skill with `read_skill_file` after the catalog description matches.
- Use **proportional** depth: a single price check is not a 20-iteration investigation.
- Present a plan and wait for approval when the research is multi-phase or high-stakes. If no approval mechanism exists, document the plan in the Methodology Note and proceed unless the user asked to wait.
- This Skill is intentionally long. That is a known token trade-off for stricter verification. Keep it as one Skill; do not split the loop.

---

# Deep Research Framework (Agora)

> **Legacy note:** before Agora Skills, this was registered in Active Memory. Now use `{skill_catalog}` + Skill description. Optional leftover AM example:
> ```
