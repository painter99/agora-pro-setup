# Skill: Active Memory Anatomy

## Catalog description

What belongs in Active Memory, quotas, anti-patterns, and System-template integration.

> Install this file into Agora **Saved Skills** (flat name, no directories).
> The body below is the operational contract. `Active Memory Anatomy` in the Skill Catalog should stay short.
...
On Agora 2.1 custom templates, Active Memory is injected **only**
where you place `{active_memory}` in the **System** template. Agora
does not silently append it. The AM is **not** a System Prompt — it
is a separate plain-text store resolved into that variable.
...
| `{active_memory}` | **Content of Active Memory** | System template — **required** on custom prompts |
...
injected raw via `{active_memory}`. If you need date awareness in AM, use the Status
