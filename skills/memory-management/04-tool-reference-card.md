# Skill: Tool Reference Card

## Catalog description

Exact Agora tool names for memory, conversation, web, shell, files, and images.

> Install this file into Agora **Saved Skills** (flat name, no directories).
> The body below is the operational contract. `Tool Reference Card` in the Skill Catalog should stay short.
...
## 1.4 Skill Tools (`SkillToolProvider`) — Agora 2.1+

Skills are **not** Memory files. When Skill access is enabled, Agora exposes:

| Tool | Args | When to use | DO NOT |
|---|---|---|---|
| `list_skill_files` | none / optional filters per current Agora schema | See what Skills exist | Confuse with `list_memory_files` |
| `read_skill_file` | `name` OR `names[]` | Load a matching Skill body after catalog discovery | Claim a Skill was applied without reading it |
| `create_skill_file` | `name`*, `content`*, opt `description` | New reusable procedure | Personal facts (those are Memory) |
| `edit_skill_file` | `name`* + `operation`: `replace` / `patch` / `rename` / `describe` | Surgical Skill edit | Silent full replace of a working Skill |
| `delete_skill_file` | `name`* | User-approved Skill removal | Auto-delete |

There is **no** `update_active_skill` and **no** Active Skill singleton.
Discovery is `{skill_catalog}` (name + short description only). Bodies are on-demand.
