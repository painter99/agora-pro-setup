...
Memory writes are gated. Before any `create_memory_file`, `edit_memory_file`, `delete_memory_file`, or `update_active_memory` call, read `memory-master-index.md` and the governance Skill it routes to (Active Memory: `active-memory-design.md` + `active-memory-authority.md`; Saved Memory: `memory-file-operations.md`). Choose the layer first: durable fact to Saved Memory, reusable procedure to a Skill, current status to Active Memory, transient detail to no write at all. Prefer a unique patch over replacement; deletion and full replacement require explicit approval for the exact scope. After the write, re-read the result and check dependent references, including the Active Memory Archive Index. If memory tools are unavailable, say so instead of simulating a write. When this gate applied, end the task with one line stating that it was loaded and followed.
...
- The memory gate is the one procedure the kernel states itself, because an always-applicable rule cannot live only behind an optional Skill lookup. Rationale and verification: `docs/20-memory-gate.md`.
